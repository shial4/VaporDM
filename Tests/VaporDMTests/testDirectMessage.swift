...
class testDirectMessage: XCTestCase {
    ...
    func testStatusParticipantsOnTwoRooms() {
        do {
            ...
            XCTAssertTrue(participants.contains(where: { user -> Bool in user.id == user1.id }), "Rooms missing participant id:1")
            XCTAssertTrue(participants.contains(where: { user -> Bool in user.id == user2.id }), "Rooms missing participant id:2")
            XCTAssertTrue(participants.contains(where: { user -> Bool in user.id == user3.id }), "Rooms missing participant id:3")
            XCTAssertTrue(participants.count == 3, "Rooms wrong participants number \(participants.count)")
            let testMessage = JSON(["type":"D"])
            XCTAssertNotNil(testMessage)
            let message = try DMFlowController(sender: user1, message: testMessage)
            let not: (redirect: JSON?, receivers: [User]) = try message.parseMessage()
            XCTAssertTrue(not.receivers.count == 3, "Rooms wrong participants number \(not.receivers.count)")
            XCTAssertTrue(not.receivers.contains(where: { user -> Bool in user.id == user2.id }), "Rooms missing participant id:2")
            XCTAssertTrue(not.receivers.contains(where: { user -> Bool in user.id == user3.id }), "Rooms missing participant id:3")
